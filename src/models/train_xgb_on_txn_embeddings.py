...
def pos_in_top_k(y_true, y_score, k):
    k=min(k,len(y_true))
    idx=np.argsort(-y_score)[:k]
...
def load_split(processed: Path, split: str):
    ...
    emb  = pd.read_parquet(processed / "static" / f"txn_embeddings_transactions_{split}.parquet")
...
def main():
    ...
    ap.add_argument("--run_name", type=str, default="xgb_txemb_v1")
    args=ap.parse_args()
    ...
    X_test,y_test,ids = load_split(processed,"test")
    ...
    ranked=pd.DataFrame({"customer_id":ids,"label":y_test,"score":p_test}).sort_values("score",ascending=False)
